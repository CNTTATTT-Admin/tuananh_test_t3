...
public class ClassRoomController {
    ...
    public ResponseEntity<ClassRoom> getClassRoomById(@PathVariable("id") Long id) throws IdInvalidException {
        ...
        if (user != null
                && (user.getRole().getName().equals("ROLE_STUDENT") || user.getRole().getName().equals("STUDENT"))) {
            ClassStudent membership = this.classRoomService.findByUserAndClass(user.getId(), id);
            if (membership == null || membership.getStatus() != ClassStatusEnum.APPROVED) {
                throw new IdInvalidException("Bạn chưa được duyệt vào lớp học này!");
            }
        }
    ...
    public ResponseEntity<?> deleteClassRoomById(@PathVariable("id") Long id) throws IdInvalidException {
        ClassRoom classRoom = this.classRoomService.getByClassRoomId(id);
        if (classRoom == null) {
            throw new IdInvalidException("class room not found");
        }
    ...
    public ResponseEntity<?> deleteClassRoomById(@PathVariable("classId") Long classId,
    ...
    public ResponseEntity<?> rejectedStudent(@RequestBody ReqUpdateStudentClassDTO req) throws IdInvalidException {
        ...
        return ResponseEntity.ok().body("rejected success");
    }
    ...
    public ResponseEntity<?> joinClassRoom(@RequestBody ReqJoinClassDTO req) throws IdInvalidException {
        ...
        this.classRoomService.joinClass(req.getClassCode());
        return ResponseEntity.ok().body("Yêu cầu đã được gửi tới Giảng viên!");
    }

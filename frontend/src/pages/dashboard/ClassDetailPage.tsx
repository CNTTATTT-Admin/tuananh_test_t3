...
export default function ClassDetailPage() {
  ...
  const getSeverity = (percentage: number): string => {
    const level = levels.find(l => percentage >= l.min && percentage <= l.max);
    if (!level) return "safe";

    // Map colors or names to severity keys if needed, 
    // but here we can just return the level name as lowercase
  ...
  const handleSaveLevels = async () => {
    ...
    try {
      ...
      // Validate contiguous ranges before saving
      const sorted = [...editingLevels].sort((a, b) => a.min - b.min);
      for (let i = 0; i < sorted.length - 1; i++) {
        if (sorted[i].max >= sorted[i + 1].min) {
          toast.error(`Mức '${sorted[i].levelName}' và '${sorted[i + 1].levelName}' bị chồng chéo!`);
          return;
        }
      }

      toast.loading("Đang cập nhật ma trận đạo văn...", { id: "saving-levels" });

      // Save all levels at once to handle overlaps and deletions correctly
      const thresholdsToSave = editingLevels.map(l => ({
      ...
      await classService.saveAllThresholds(clsId, thresholdsToSave);

      toast.success("Đã cập nhật ma trận đạo văn!", { id: "saving-levels" });
      setLevelsDialogOpen(false);
      fetchData();
    } catch (error: any) {
      toast.error(error.response?.data?.message || error.message || "Không thể cập nhật các ngưỡng", { id: "saving-levels" });
    }
  };
